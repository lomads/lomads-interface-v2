...
export const DAOProvider = ({ privateRoute = false, children }: any) => {
  ...
  useEffect(() => {
    if (user?._id) dispatch(createAccountAction({ token }));
  }, [user?._id]);
    ...
    if (provider && account && DAO?.url) {
      if (DAO?.sbt) {
              ...
              if (
                ...
                (DAO?.sbt?.whitelisted &&
                  _find(
                    DAO?.members,
                    (member: any) =>
                      toChecksumAddress(member.member.wallet) === account
                  ))
              ) {
                if (+DAO?.sbt?.version >= 2)
                  navigate(`/${DAO?.url}/mint/${DAO?.sbt?.address}`);
                else navigate(`/${DAO?.url}/mint/v1/${DAO?.sbt?.address}`);
              } else {
                // NOT WHITELISTED
        ...
        if (
          !_find(
            DAO?.members,
            (member: any) => toChecksumAddress(member.member.wallet) === account
          )
        )
  ...
  useEffect(() => {
    if (
      window.location.pathname.indexOf("attach-safe/new") == -1 &&
      DAO &&
      (!DAO.safes || (DAO.safes && DAO.safes.length == 0))
    ) {
  ...
  return (
    <DAOContext.Provider value={contextProvider}>
      {privateRoute && (!DAOList || (daoURL && !DAO)) ? (
        <FullScreenLoader />
      ) : (
        children
      )}
