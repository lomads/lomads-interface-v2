...
export default ({ open, onClose }: { open: boolean; onClose: any }) => {
  const { account, provider, chainId: currentChainId } = useWeb3Auth()
  ...
  const [updateContractLoading, setUpdateContractLoading] = useState<
    boolean | null
  >(null)
  const { updateContract, withdraw } = useMintSBT(
    _get(contract, 'address', ''),
    _get(contract, 'version', '')
  )
      ...
      setState((prev: any) => {
        return {
          ...
          priced:
            contract?.mintPrice &&
            contract?.mintPrice !== '' &&
            parseFloat(contract?.mintPrice) > 0
              ? true
              : false,
          price: {
            token: _get(
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      if (
        (+contract?.version >= 1 &&
          state?.price?.token !== prevState?.price?.token) ||
        state?.price?.value !== prevState?.price?.value
      ) {
      ...
      return await axiosHttp
        .patch(`contract/${contract?.address}`, {
          daoId: DAO?._id,
  ...
  const handleContactChange = (key: string) => {
    setState((prev: any) => {
      return {
        ...prev,
        contact:
          prev?.contact.indexOf(key) > -1
            ? prev.contact.filter((c: string) => c !== key)
            : [...prev?.contact, key],
      }
    })
  }
    ...
    <Box>
      ...
      <Box sx={{ pb: 6 }}>
        ...
        >
          ...
          >
            {' '}
            Launch '<span style={{ fontWeight: '700' }}>
              Soulbound Tokens
            </span>{' '}
            (SBTs)' with ease and streamline app permissions (eg. Discord,
            Notion, Github, and Snapshot). Don't worry,{' '}
            <span style={{ fontWeight: '700' }}>
              member data is always secure
            </span>{' '}
            with encryption on SBT.
          </Typography>
        ...
        {!contract ? (
          ...
          >
            ...
            >
              The organisation doesn't have a token yet
            </Typography>
            <Button
              sx={{ mt: 3 }}
              onClick={() => {
              ...
              >
                {chainId && (
                  <Box
                    onClick={() =>
                      window.open(
                        `${_get(CHAIN_INFO, chainId).explorer}address/${
                          contract.address
                        }`
                      )
                    }
                  ...
                  >
                    {contract?.image ? (
                      ...
                      <img
                        style={{ width: 40, borderRadius: 10, height: 40 }}
                        src={PT}
                      />
                    )}
                  </Box>
                ...
                {chainId && (
                  ...
                  >
                    <img
                      ...
                      src={_get(CHAIN_INFO, chainId).logoUrl}
                    />
                  </Box>
                )}
              </Box>
 ...
 */}
            ...
            <Box className={classes.paperDetailsSocial}>
              <Box>
                ...
                <Typography
                  variant='body2'
                  className={clsx(classes.socialText, {
                ...
                >
                  ...
                  <Typography
                    ...
                    className={classes.socialText}
                  >
                    Please select if you intend to use access-controlled github.
                  </Typography>
                </Box>
              </Box>
            </Box>
